...
function GameManager(){
    ...
    var phonegapModeOn = (typeof PhoneGap !== "undefined");
    ...
    var landScapeModeOn         = (isPhonegapModeOn() || isMobileDevice()) ? true : false;
    ...
    var phonegapAccelerometerSupport    = false;
    
    //sounds (initialized in initSounds)
    var ACTIVATE_SOUND_EFFECTS      = true;
    var DISABLE_PHONEGAP_SOUND_SUPPORT = true;//this is temporary, until PGLowLatencyAudio is upgraded to phonegap 1.5
    var SOUND_BALL_COLLISION_FILE   = "audio/bounce-ball.wav";
    var SOUND_BORDER_COLLISION_FILE = "audio/bounce-rail.wav";
    var SOUND_EXPLOSION_FILE        = "audio/explosion.wav";
    var html5SoundSupport           = false;
    var phonegapSoundSupport        = false;
    var sounds                      = {};
    
    //screens
    var SCREEN_LEVEL_DURATION       = 2500;
    var SCREEN_FLASH_RATE           = 40;
    ...
    this.init = function(){
        ...
        Stage.prepareStage(function(stageCanvas){
            ...
            if(isPhonegapModeOn() == false && html5DeviceMotionSupport == false)
                alert("Your browser/device doesn't have accelerometer support.\nIf you have an Android device, you can install the app from the market.");
        });
    ...
    this.initOrientationCss = function(){
        if(isLandScapeModeOn()){
            jQuery('#viewporter').addClass('landscape');
        }
        else{
            jQuery('#viewporter').addClass('portrait');
        }
    }
    ...
    this.initSounds = function(){
        ...
        phonegapSoundSupport    = isPhonegapModeOn() && ACTIVATE_SOUND_EFFECTS && DISABLE_PHONEGAP_SOUND_SUPPORT == false;
        console.info('html5SoundSupport',html5SoundSupport,'phonegapSoundSupport',phonegapSoundSupport);
        ...
        else if(phonegapSoundSupport == true){
//            sounds['soundBallCollision'] = new Media('/android_asset/www/'+SOUND_BALL_COLLISION_FILE,function(){console.info('Media.success')},function(e){console.info('Media.error',e)});
//            sounds['soundBorderCollision'] = new Media('/android_asset/www/'+SOUND_BORDER_COLLISION_FILE,function(){console.info('Media.success')},function(e){console.info('Media.error',e)});
            console.info('PGLowLatencyAudio - preloading ...');
            PGLowLatencyAudio.preloadFX('soundBallCollision', SOUND_BALL_COLLISION_FILE);
            PGLowLatencyAudio.preloadFX('soundBorderCollision', SOUND_BORDER_COLLISION_FILE);
            PGLowLatencyAudio.preloadFX('soundExplosion', SOUND_EXPLOSION_FILE);
            console.info('PGLowLatencyAudio - end preloading ...');
        }
    }
    
    this.initMotionListeners = function(){
        
        var phonegapAccelerometerWatchId = null;//phonegap needs this var like a setInterval id
        if(isPhonegapModeOn()){
            phonegapAccelerometerSupport = true;
            document.addEventListener("deviceready", function(){
                phonegapAccelerometerWatchId = navigator.accelerometer.watchAcceleration(function(acceleration){
                    //success callback for accelerometer
                    inputX = -acceleration.x*DEVICEMOTION_INPUT_RATIO;
                    inputY = acceleration.y*DEVICEMOTION_INPUT_RATIO;
                }, function(e){
                    //error callback for accelerometer
            ...
            if(!window.DeviceMotionEvent){
                ...
                window.addEventListener('devicemotion', function(event){
                    inputX = event.accelerationIncludingGravity.x*DEVICEMOTION_INPUT_RATIO;
                    inputY = -event.accelerationIncludingGravity.y*DEVICEMOTION_INPUT_RATIO;
                },false)
            }
        }
        console.info('phonegapAccelerometerSupport',phonegapAccelerometerSupport,'html5DeviceMotionSupport',html5DeviceMotionSupport);
    ...
    */
    ...
    this.addGameScreenListeners = function(){
        console.info('addGameScreenListeners');
        screenOn = null;
        var self = this;
        var eventTargeted = isMobileDevice() ? 'touchstart touchend' : 'click';
        jQuery('canvas').bind(eventTargeted,function(e){
...
//                setTimeout(function(){self.initWelcomeScreens('home');},300);//setTimeout to avoid multitap
        ...
        if(isPhonegapModeOn() == false && isMobileDevice())
            jQuery('canvas').bind('touchmove',function(e){e.preventDefault();});
    ...
    this.removeGameScreenListeners = function(){
        console.info('removeGameScreenListeners');
        var eventTargeted = isMobileDevice() ? 'touchstart touchend' : 'click';
        jQuery('canvas').unbind(eventTargeted);
        
        if(isPhonegapModeOn() == false && isMobileDevice())
            jQuery('canvas').unbind('touchmove',function(e){e.preventDefault();});
    }
    
    this.addWelcomeScreenListeners = function(){
        console.info('addWelcomeScreenListeners');
        var self = this;
        var eventTargeted = isMobileDevice() ? 'click' : 'click';
        jQuery('canvas').bind(eventTargeted,function(e){
            switch(screenOn){
                case 'welcome':
    ...
    this.removeWelcomeScreenListeners = function(){
        console.info('removeWelcomeScreenListeners');
        var eventTargeted = isMobileDevice() ? 'click' : 'click';
        jQuery('canvas').unbind(eventTargeted);
    ...
    this.initWelcomeScreens = function(screen){
        screen = screen || 'home';
        console.info('GameManager.initWelcomeScreens','canvas',canvas);
...
//        this.nextLevel();
    ...
    this.initGame = function(){
        console.info('GameManager.initGame');
    ...
    this.nextLevel = function(sameLevel){
        console.info('GameManager.nextLevel');
        var self = this;
        if(sameLevel !== true)
            level++;
        ...
        playInfosMessage(drawLevelScreen,SCREEN_LEVEL_DURATION,function(){
            pauseAllowed = true;
            self.startLoop();
            jQuery(canvas).removeClass('pause');
            jQuery(canvas).addClass('loop');
        });
    }
    
    this.prepareLevel = function(level,sameLevel){
        console.info('GameManager.prepareLevel');
        ...
        console.info('maxEnemiesOnScreen',maxEnemiesOnScreen);
    ...
    this.startLoop = function(){
        console.info('GameManager.startLoop');
    ...
    this.stopLoop = function(){
        console.info('GameManager.stopLoop');
        //prevent double stop loop
        if(looping == true){
            console.info('stopLoop');
            looping = false;
    ...
    this.toggleLoop = function(){
        ...
        if(looping == true){
            this.stopLoop();
            jQuery(canvas).removeClass('loop');
            jQuery(canvas).addClass('pause');
            drawPauseScreen();
        }
        else{
            jQuery(canvas).removeClass('pause');
...
//                    }
    ...
    function armBomb(bomb){
        bomb.blink();
        bomb.armed = true;
        console.info('armed',currentTimeStamp);
    }
    ...
    function explodeBomb(bomb){
        bomb.explode();
        console.info('bomb explode',currentTimeStamp);
    }
    ...
    function explodeEnemy(enemy){
        ...
        playSoundExplosion();
        console.info('enemy explode',currentTimeStamp);
    }
    ...
    function explodePlayer(player){
        ...
        playSoundExplosion();
        console.info('player explode',currentTimeStamp);
    }
...
//                console.info('html5 sound.play error ',e);
            ...
            PGLowLatencyAudio.play(soundId,function(){console.info('success play '+soundId)},function(){console.info('failure play '+soundId)});
        }
    }
    ...
    function drawTapScreenNotificationMessage(dy){
        drawCenteredText('TAP SCREEN TO DROP BOMBS',24,370,0,dy,BOMB_COLOR);
    }
    ...
    function preloadImages(){
        ...
        if(isWideScreen()){
            images['bandeau'].src = './img/topheman-bombs-bandeau.png';
        }
        else{
            images['bandeau'].src = './img/topheman-bombs-bandeau-min.png';
        }
    ...
    var welcomeScreensDrawingFunction = {
        ...
        'instructions1': function(){
            console.info('draw instructions1');
        ...
        'instructions2': function(){
            console.info('draw instructions2');
        ...
        'instructions3': function(){
            console.info('draw instructions3');
        ...
        'instructions4': function(){
            console.info('draw instructions4');
        ...
        'instructions5': function(){
            console.info('draw instructions5');
     ...
     * @return {Boolean}
     */
    function isPhonegapModeOn(){
        return phonegapModeOn;
    }
